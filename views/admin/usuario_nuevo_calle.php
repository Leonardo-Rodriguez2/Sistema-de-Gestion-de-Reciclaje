<?php
// views/admin/usuario_nuevo_calle.php
$user = check_dashboard_access([1]);

// Fetch streets for the dropdown
$callesStmt = $pdo->query("SELECT c.id, c.nombre, b.nombre as barrio_nombre 
                            FROM calles c JOIN barrios b ON c.barrio_id = b.id 
                            ORDER BY b.nombre, c.nombre ASC");
$calles_lista = $callesStmt->fetchAll(PDO::FETCH_ASSOC);

$title = "Registrar Encargado de Calle - EcoCusco";
$header_title = "Nuevo Encargado de Calle";
$header_subtitle = "Asigna un nuevo supervisor para una calle específica dentro de un barrio.";
...
<div class="form-container">
    <form action="router.php" method="POST" class="premium-form">
        ...
        <div class="form-section">
            <h3><span class="icon">📍</span> Asignación de Calle</h3>
            <div class="form-grid">
                <div class="form-group">
                ...
                <div class="form-group" style="grid-column: span 2;">
                    ...
                    <select name="calle_id" required style="width: 100%;">
                        <option value="">Seleccione una calle...</option>
                        <?php foreach ($calles_lista as $c): ?>
                            <option value="<?= $c['id'] ?>"><?= htmlspecialchars($c['barrio_nombre'] . " — " . $c['nombre']) ?></option>
                        <?php endforeach; ?>
                    </select>
                </div>
            </div>
        ...
        <div class="form-actions">
            <a href="router.php?page=usuarios&rol_id=6" class="btn-cancel">Volver a la lista</a>
            <button type="submit" class="btn-submit">Registrar Encargado</button>
        </div>
    </form>
</div>
