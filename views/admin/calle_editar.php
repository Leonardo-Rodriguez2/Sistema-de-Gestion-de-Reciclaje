...
if (!$calle) { echo "<div class='alert alert-error'>Calle no encontrada.</div>"; return; }

$barrios = $pdo->query("SELECT id, nombre FROM barrios ORDER BY nombre")->fetchAll(PDO::FETCH_ASSOC);

$title = "Editar Calle - EcoCusco";
$header_title = "Editar Calle";
$header_subtitle = "Modifica los datos de la calle seleccionada.";

ob_start();
?>
<div class="form-container" style="margin: 0 auto;">
    <form action="router.php" method="POST" class="premium-form">
        <input type="hidden" name="action" value="editar_calle">
        ...
        <div class="form-section">
            ...
            <div class="form-group">
                ...
                <input type="text" name="nombre" required value="<?= htmlspecialchars($calle['nombre']) ?>" placeholder="Ej. Avenida Los Próceres">
            </div>
            ...
            <div class="form-group" style="margin-top: 15px;">
                ...
                <select name="barrio_id" required style="width: 100%; padding: 10px; border: 1px solid #E5E7EB; border-radius: 8px; font-size: 13px;">
                    ...
                    <?php foreach($barrios as $b): ?>
                        <option value="<?= $b['id'] ?>" <?= $b['id'] == $calle['barrio_id'] ? 'selected' : '' ?>><?= htmlspecialchars($b['nombre']) ?></option>
                    <?php endforeach; ?>
                </select>
            </div>
        </div>
